...
- **主功能（IMAPIndex.js 入口）**：完全兼容 Junior 协议，直接可用。
- **附属脚本** `storkapp_dailynote/` 和 `storkapp_dailynote_pubmed/`：源自上游文献鸟 pipeline，代码里硬编码了 `dailynote/` 目录（Junior 已将该目录改名为 `knowledge/`）。这两个脚本**不被主插件自动加载**，不影响主功能。若需使用，请手动把脚本中的 `dailynote` 修改为 `knowledge` 后再独立运行。
